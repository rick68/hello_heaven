...
use {
    ...
    kernel::{
        ...
        error::Result,
        macros::module,
        pr_alert, pr_cont, pr_crit, pr_emerg, pr_err, pr_info, pr_notice, pr_warn,
        sync::{Arc, UniqueArc},
    },
};

fn arc_print() -> Result {
    let a: Arc<i32> = Arc::new(1, GFP_KERNEL)?;
    let b: UniqueArc<&str> = UniqueArc::new("hello, world", GFP_KERNEL)?;
    ...
    let a: Arc<&str> = Arc::from(b);
    let c: Arc<&str> = a.clone();
...
mod trace {
    use kernel::{declare_trace, ffi::c_int};
...
impl Drop for RustPrint {
    fn drop(&mut self) {
        pr_info!("Rust printing sample (exit)\n");
    }
}
